...
#        self.takeoffAndMoveAway()
    ...
    def CANGPSIntegrity(self):
        ...
        # (SIM_GPS1_OPTIONS bit 0 F9P + bit 1 SEC-SIG)
        ...
        try:
            for flavour, options in flavours:
                if options != 0:
                    ...
                    self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_OPTIONS', options)
                    # starve the periph GPS driver (no simulated receiver at
                    # all) so AP_GPS times out and re-detects the receiver
                    # with the new options
                    self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_TYPE', 0)
                    ...
                    self.delay_sim_time(3)
                    self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_TYPE', 1)
                ...
                self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_JAM', 1)
                self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_SPOOF', 1)
                ...
                self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_JAM', 0)
                self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_SPOOF', 0)
                self.wait_gnss_integrity(nominal, timeout=30)
        finally:
            # make sure the periph is left in its nominal state
            self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_JAM', 0)
            self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_SPOOF', 0)
            self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_OPTIONS', 0)
            self.dronecan_periph_param_set(node, gps1_nodeid, 'SIM_GPS1_TYPE', 1)
            node.close()
