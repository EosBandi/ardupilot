...
void GPS_UBlox::publish(const GPS_Data *d)
{
    ...
    status.flags2 = (_sitl->gps[instance].spoof == 1 ? 2U : 1U) << 3;
    ...
    // send MON-HW (or MON-RF for F9P) at 1Hz with the simulated jamming state
    {
        ...
        if ((int32_t)(now_ms - _next_mon_send_ms) >= 0) {
            ...
            const bool jammed = _sitl->gps[instance].jam == 1;
            ...
            if (sec_sig_firmware() && _sec_sig_rate != 0) {
                ...
                const uint8_t spf_state = (_sitl->gps[instance].spoof == 1) ? 2 : 1;
...
{
    return (_sitl->gps[instance].options & static_cast<int32_t>(SITL::SIM::GPSOptions::UBX_SEC_SIG)) != 0;
}
