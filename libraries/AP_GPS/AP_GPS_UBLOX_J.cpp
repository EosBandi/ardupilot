...
AP_GPS_UBLOX_J::AP_GPS_UBLOX_J(AP_GPS &_gps, AP_GPS::GPS_State &_state, AP_HAL::UARTDriver *_port, AP_GPS::GPS_Role _role) :
 ...
 */
const AP_GPS_UBLOX_J::config_list AP_GPS_UBLOX_J::config_MB_Base_uart1[] {
...
const AP_GPS_UBLOX_J::config_list AP_GPS_UBLOX_J::config_MB_Base_uart2[] {
 ...
 */
const AP_GPS_UBLOX_J::config_list AP_GPS_UBLOX_J::config_MB_Rover_uart1[] {
 { ConfigKey::CFG_UART2OUTPROT_RTCM3X, 0},
 { ConfigKey::CFG_UART1INPROT_RTCM3X, 1},
...
const AP_GPS_UBLOX_J::config_list AP_GPS_UBLOX_J::config_MB_Rover_uart2[] {
 { ConfigKey::CFG_UART2_ENABLED, 1},
 { ConfigKey::CFG_UART2_BAUDRATE, 460800},
 { ConfigKey::CFG_UART2OUTPROT_RTCM3X, 0},
 ...
 */
const AP_GPS_UBLOX_J::config_list AP_GPS_UBLOX_J::config_M10[] {
...
void
AP_GPS_UBLOX_J::_request_next_config(void)
{
...
void
AP_GPS_UBLOX_J::_verify_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate) {
    ...
    case CLASS_NAV:
        ...
        case MSG_DOP:
            ...
            break;
        case MSG_NAV_SAT:
            desired_rate = RATE_SAT;
            config_msg_id = CONFIG_RATE_SAT;
            break;
        default:
            return;
        }
...
void
AP_GPS_UBLOX_J::_request_port(void)
{
...
// Private Methods /////////////////////////////////////////////////////////////
void AP_GPS_UBLOX_J::log_mon_hw(void)
{
...
void AP_GPS_UBLOX_J::log_mon_hw2(void)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
#if UBLOX_TIM_TM2_LOGGING
void AP_GPS_UBLOX_J::log_tim_tm2(void)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
#if UBLOX_RXM_RAW_LOGGING
void AP_GPS_UBLOX_J::log_rxm_raw(const struct ubx_rxm_raw &raw)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
void AP_GPS_UBLOX_J::log_rxm_rawx(const struct ubx_rxm_rawx &raw)
{
#if HAL_LOGGING_ENABLED
    if (!should_log()) {
        return;
...
void AP_GPS_UBLOX_J::unexpected_message(void)
{
...
uint8_t AP_GPS_UBLOX_J::config_key_size(ConfigKey key) const
{
    // step over the value
 ...
 */
int8_t AP_GPS_UBLOX_J::find_active_config_index(ConfigKey key) const
{
#if GPS_MOVING_BASELINE
...
bool
AP_GPS_UBLOX_J::_parse_gps(void)
{
    if (_class == CLASS_ACK) {
...
#ifdef HAL_GPIO_PPS
            hal.gpio->attach_interrupt(HAL_GPIO_PPS, FUNCTOR_BIND_MEMBER(&AP_GPS_UBLOX_J::pps_interrupt, void, uint8_t, bool, uint32_t), AP_HAL::GPIO::INTERRUPT_FALLING);
#endif
    ...
    case MSG_STATUS:
        ...
        if (_buffer.status.fix_status & NAV_STATUS_FIX_VALID) {
            if( (_buffer.status.fix_type == AP_GPS_UBLOX_J::FIX_3D) &&
                (_buffer.status.fix_status & AP_GPS_UBLOX_J::NAV_STATUS_DGPS_USED)) {
                next_fix = AP_GPS::GPS_OK_FIX_3D_DGPS;
            }else if( _buffer.status.fix_type == AP_GPS_UBLOX_J::FIX_3D) {
                next_fix = AP_GPS::GPS_OK_FIX_3D;
            }else if (_buffer.status.fix_type == AP_GPS_UBLOX_J::FIX_2D) {
    ...
    case MSG_SOL:
        ...
        if (_buffer.solution.fix_status & NAV_STATUS_FIX_VALID) {
            if( (_buffer.solution.fix_type == AP_GPS_UBLOX_J::FIX_3D) &&
                (_buffer.solution.fix_status & AP_GPS_UBLOX_J::NAV_STATUS_DGPS_USED)) {
                next_fix = AP_GPS::GPS_OK_FIX_3D_DGPS;
            }else if( _buffer.solution.fix_type == AP_GPS_UBLOX_J::FIX_3D) {
                next_fix = AP_GPS::GPS_OK_FIX_3D;
            }else if (_buffer.solution.fix_type == AP_GPS_UBLOX_J::FIX_2D) {
                next_fix = AP_GPS::GPS_OK_FIX_2D;
            }else{
                next_fix = AP_GPS::NO_FIX;
                state.status = AP_GPS::NO_FIX;
    ...
    case MSG_RELPOSNED:
        {
            ...
            MB_Debug("RELPOSNED flags: %lx valid: %lx invalid: %lx\n", (unsigned long)_buffer.relposned.flags, (unsigned long)valid_mask, (unsigned long)invalid_mask);
...
#ifdef HAL_GPIO_PPS
void
AP_GPS_UBLOX_J::pps_interrupt(uint8_t pin, bool high, uint32_t timestamp_us)
{
    _last_pps_time_us = timestamp_us;
}

void
AP_GPS_UBLOX_J::set_pps_desired_freq(uint8_t freq)
{
    _pps_freq = freq;
    _unconfigured_messages |= CONFIG_TP5;
}
 ...
 */
void
AP_GPS_UBLOX_J::_update_checksum(uint8_t *data, uint16_t len, uint8_t &ck_a, uint8_t &ck_b)
{
    while (len--) {
        ck_a += *data;
        ck_b += ck_a;
 ...
 */
bool
AP_GPS_UBLOX_J::_send_message(uint8_t msg_class, uint8_t msg_id, const void *msg, uint16_t size)
{
 ...
 */
bool
AP_GPS_UBLOX_J::_request_message_rate(uint8_t msg_class, uint8_t msg_id)
{
 ...
 */
bool
AP_GPS_UBLOX_J::_configure_message_rate(uint8_t msg_class, uint8_t msg_id, uint8_t rate)
{
 ...
 */
bool
AP_GPS_UBLOX_J::_configure_valset(ConfigKey key, const void *value, uint8_t layers)
{
    if (!supports_F9_config()) {
        return false;
    }
 ...
 */
bool
AP_GPS_UBLOX_J::_configure_valget(ConfigKey key)
{
    if (!supports_F9_config()) {
        return false;
    }
 ...
 */
bool
AP_GPS_UBLOX_J::_configure_config_set(const config_list *list, uint8_t count, uint32_t unconfig_bit, uint8_t layers)
{
 ...
 */
bool
AP_GPS_UBLOX_J::_detect(struct UBLOX_detect_state &state, uint8_t data)
{
reset:
	switch (state.step) {
        case 1:
...
void
AP_GPS_UBLOX_J::_request_version(void)
{
    _send_message(CLASS_MON, MSG_MON_VER, nullptr, 0);
}

void
AP_GPS_UBLOX_J::_configure_rate(void)
{
...
void
AP_GPS_UBLOX_J::broadcast_configuration_failure_reason(void) const {
 ...
 */
bool AP_GPS_UBLOX_J::get_lag(float &lag_sec) const
{
    switch (_hardware_generation) {
...
void AP_GPS_UBLOX_J::Write_AP_Logger_Log_Startup_messages() const
{
#if HAL_LOGGING_ENABLED
...
void AP_GPS_UBLOX_J::_check_new_itow(uint32_t itow)
{
    check_new_itow(itow, _payload_length + sizeof(ubx_header) + 2);
}

// support for retrieving RTCMv3 data from a moving baseline base
bool AP_GPS_UBLOX_J::get_RTCMV3(const uint8_t *&bytes, uint16_t &len)
{
#if GPS_MOVING_BASELINE
    if (rtcm3_parser) {
...
void AP_GPS_UBLOX_J::clear_RTCMV3(void)
{
#if GPS_MOVING_BASELINE
    if (rtcm3_parser) {
...
bool AP_GPS_UBLOX_J::is_healthy(void) const
{
...
bool AP_GPS_UBLOX_J::supports_F9_config(void) const
{
